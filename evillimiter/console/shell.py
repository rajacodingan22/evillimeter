...
def execute(command, root=True):
    return subprocess.call(("sudo " if root else "") + command, shell=True)


def execute_suppressed(command, root=True):
    return subprocess.call(
        ("sudo " if root else "") + command,
        shell=True,
...
def output(command, root=True):
    return subprocess.check_output(
        ("sudo " if root else "") + command, shell=True
    ).decode("utf-8")


def output_suppressed(command, root=True):
    return subprocess.check_output(
        ("sudo " if root else "") + command, shell=True, stderr=subprocess.DEVNULL
    ).decode("utf-8")
